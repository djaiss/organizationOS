<aside class="flex-col border-b border-gray-200 bg-white px-4 py-4 sm:flex sm:rounded-bl-lg sm:border-r sm:border-b-0">
  <nav class="flex flex-col gap-1">
    ...
    <a href="{{ route('settings.profile.edit') }}" class="{{ request()->routeIs('settings.profile.edit') ? 'bg-gray-100 font-medium text-gray-900' : 'text-gray-700 hover:bg-gray-50' }} flex items-center gap-3 rounded-lg px-2 py-1">
      <x-phosphor-user class="h-4 w-4 {{ request()->routeIs('settings.profile.edit') ? 'text-orange-400' : 'text-gray-500' }}" />
      {{ __('Profile') }}
    </a>
